...
def HR_dashboard(request):
    #company = Company.objects.all().filter(user_id = request.user.id)
    #print(company)
    #job_post = Listing.objects.all().filter(company = company).order_by('-publish_date')
    #user_appliers = Apply.objects.all().filter(
    #    user_id = user.id).order_by('-apply_date')
